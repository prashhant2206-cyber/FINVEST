...
export class ScoringEngine {
  /**
   * Normalizes a numeric array with winsorization and bounds (0 to 100).
   * higherIsBetter = true => highest value gets 100, lowest gets 0.
   * higherIsBetter = false => lowest value gets 100, highest gets 0.
   */
  private static normalizeMetric(
    ...
    // Percentile rank approach with smooth linear scaling
    return cleanValues.map((val) => {
      // Find percentile rank in current distribution
      let countBelow = 0;
      let countEqual = 0;
      for (let s of sorted) {
        if (s < val) countBelow++;
      ...
      // Percentile formula: (countBelow + 0.5 * countEqual) / N
  ...
  /**
   * Evaluates if a company passes user-defined screening filters
   */
  ...
  /**
   * Computes normalized scores for an entire universe of companies dynamically based on user weights.
   */
  public static calculateScores(
    ...
    // Normalizing sub-metrics across the universe
    ...
    // Normalize weights to ensure 100% distribution mathematically
