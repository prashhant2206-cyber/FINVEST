import { Company, MLCluster, AnomalyRecord } from '../../src/types';

export class MLEngine {
  /**
   * K-Means clustering across 7 key financial dimensions.
   */
  public static runClustering(companies: Company[], k: number = 4): { clusters: MLCluster[]; companyClusterMap: Map<string, MLCluster> } {
    ...
    // Normalize features for distance calculation (0 to 1 min-max)
    ...
    // Initialize k centroids deterministically or spread out
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      ...
      // 1. Assign to nearest centroid (Euclidean distance)
    ...
    // Build interpreted clusters based on actual financial centroid values
    ...
    for (let c = 0; c < actualK; c++) {
      ...
      // Dynamic label heuristics based on centroid profile
  ...
  /**
   * Detects accounting & financial anomalies requiring analyst investigation.
   */
  public static detectAnomalies(companies: Company[]): AnomalyRecord[] {
    ...
    companies.forEach((comp) => {
      ...
      // Anomaly 1: Profit growth vs Operating Cash Flow divergence (CFO / PAT < 0.65)
      if (m.cfoToPatRatio < 0.65) {
        anomalies.push({
          ...
          severity: 'high',
          explanation: `Net profit is not translating into operating cash flows. 3Y profit growth is outpacing cash collection, indicating potential aggressive revenue recognition or aggressive channel inventory buildup.`,
          suggestedAction: 'Investigate quality of earnings, debtor aging schedules, and cash flow statement reconciliations.',
        });
      }

      // Anomaly 2: Working Capital Days ballooning (> 120 days or negative divergence)
      ...
      // Anomaly 3: High Leverage coupled with Low Interest Coverage (D/E > 0.8 and Int Coverage < 10x for non-banks)
      ...
      // Anomaly 4: High P/E Disconnect vs Single-Digit Growth (PE > 65 with Rev CAGR < 10%)
      ...
      // Anomaly 5: Historical margin volatility in recent years
      if (h && h.length >= 3) {
